...
export function VoiceRecorder({ onTranscript, onClose }: Props) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setError("Microphone denied. Allow mic in browser settings then try again.");
      setState("error");
      return;
    }
    ...
    rec.onstop = async () => {
      ...
      try {
        ...
        if (data.text?.trim()) {
          ...
          setError(data.error || "Could not transcribe. Try speaking louder.");
          setState("error");
        }
      } catch (e: any) {
    ...
    <div>
      ...
      {state === "idle" && (
        <div style={{ display:"flex", justifyContent:"center", padding:"0 20px 16px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"6px 16px", borderRadius:"20px", background:"rgba(93,202,165,0.1)", border:"1px solid rgba(93,202,165,0.25)" }}>
            <Sparkles size={11} style={{ color:"#5DCAA5" }} />
            <span style={{ fontSize:"11px", color:"#5DCAA5", fontFamily:"var(--font-body)", fontWeight:500 }}>Whisper AI · Works in all browsers</span>
          </div>
        </div>
      )}
      ...
      <div style={{ background:"var(--bg)", minHeight:"90px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"8px", padding:"20px" }}>
        {state === "recording" && (
          <>
            ...
            <p style={{ fontSize:"11px", color:"var(--text-muted)" }}>Recording… tap Stop & Save when done</p>
          </>
        )}
        {state === "processing" && (
          <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
            <div style={{ width:"20px", height:"20px", borderRadius:"50%", border:"2px solid var(--border)", borderTopColor:"#5DCAA5", animation:"spin 0.8s linear infinite" }} />
            <span style={{ fontSize:"13px", color:"var(--text-secondary)" }}>Transcribing with Whisper AI…</span>
          </div>
        )}
        {state === "error" && (
        ...
        {state === "done" && (
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            <Check size={18} style={{ color:"#5DCAA5" }} />
            <span style={{ fontSize:"13px", color:"var(--text-secondary)" }}>{aiEnhanced ? "✦ AI cleaned up" : "Ready to insert"}</span>
          </div>
        )}
        {state === "idle" && (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"6px" }}>
            <Mic size={28} style={{ color:"var(--text-muted)", opacity:0.25 }} />
            <p style={{ fontSize:"12px", color:"var(--text-muted)", textAlign:"center" }}>
              Record your voice — Whisper AI will transcribe it
            </p>
          </div>
        )}
      </div>
      ...
      <div style={{ display:"flex", gap:"8px", padding:"0 20px 20px" }}>
        ...
        {state === "done" && (
          <>
            ...
            <button onClick={() => { setState("idle"); setTranscript(""); setAiEnhanced(false); }}
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:50, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px", background:"rgba(0,0,0,0.6)", backdropFilter:"blur(8px)" }}>
      <div style={{ width:"100%", maxWidth:"420px", background:"var(--surface)", borderRadius:"20px", overflow:"hidden", boxShadow:"var(--shadow-lg)" }}>
        <div style={{ borderBottom:"1px solid var(--border)" }}>{header}</div>
        {body}
      </div>
    </div>
