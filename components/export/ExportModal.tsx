...
const LAYOUT_TEMPLATES: { id: LayoutTemplate; label: string; desc: string; icon: string }[] = [
  { id: "none",    label: "Note",        desc: "Your note content",         icon: "¶" },
  { id: "lined",   label: "Lined",       desc: "Classic ruled lines",       icon: "≡" },
  { id: "grid",    label: "Grid",        desc: "Graph paper style",         icon: "⊞" },
  { id: "bullet",  label: "Bullet",      desc: "Dot grid journal",          icon: "·" },
  { id: "cornell", label: "Cornell",     desc: "Notes + margin + summary",  icon: "▤" },
  { id: "weekly",  label: "Weekly",      desc: "7-day planner layout",      icon: "⊟" },
];
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4"
      ...
      <div className="w-full max-w-2xl rounded-2xl overflow-hidden animate-scale"
        ...
        <div className="flex flex-col md:flex-row overflow-auto" style={{ maxHeight: "calc(90vh - 130px)" }}>
          {/* Options */}
          <div className="p-5 space-y-5 md:w-64 flex-shrink-0 overflow-y-auto">

            {/* Style Template */}
            <div>
              <p style={{ fontSize: "10px", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "10px" }}>Style</p>
            ...
            {/* Layout Template */}
            <div>
              <p style={{ fontSize: "10px", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "10px" }}>Layout</p>
              <div className="grid grid-cols-3 gap-1.5">
                {LAYOUT_TEMPLATES.map(l => (
                  <button key={l.id} onClick={() => setLayoutTemplate(l.id)}
                    className="flex flex-col items-center gap-1 p-2 rounded-xl transition-all"
                    style={{
                      background: layoutTemplate === l.id ? "var(--text)" : "var(--surface-hover)",
                      border: `1px solid ${layoutTemplate === l.id ? "var(--text)" : "var(--border-light)"}`,
                    }}>
              ...
              {layoutTemplate !== "none" && (
                <p style={{ fontSize: "10px", color: "var(--text-muted)", marginTop: "6px", textAlign: "center" }}>
                  {LAYOUT_TEMPLATES.find(l => l.id === layoutTemplate)?.desc}
                </p>
              )}
            </div>
              ...
              {showAIGen && (
                <div className="mt-3 space-y-3 animate-fade">
                  <textarea
                    ...
                    placeholder="e.g. weekly standup, book review, travel plan…"
                    rows={3}
                    style={{
...
// ── Layout renderers ──────────────────────────────────────────────
function LinedLayout({ accent, bg }: { accent: string; bg: string }) {
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
      {Array.from({ length: 28 }).map((_, i) => (
        <div key={i} style={{
          position: "absolute", left: "48px", right: "32px",
          top: `${120 + i * 28}px`, height: "1px",
      ...
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        <defs>
          <pattern id="grid" width={size} height={size} patternUnits="userSpaceOnUse">
            <path d={`M ${size} 0 L 0 0 0 ${size}`} fill="none" stroke={accent} strokeWidth="0.4" opacity="0.2" />
          </pattern>
        </defs>
...
function DotGridLayout({ accent }: { accent: string }) {
  ...
  for (let r = 0; r < 30; r++) {
    for (let c = 0; c < 25; c++) {
      dots.push({ x: 30 + c * spacing, y: 30 + r * spacing });
    }
  }
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        {dots.map((d, i) => (
          <circle key={i} cx={d.x} cy={d.y} r="1" fill={accent} opacity="0.2" />
        ))}
      </svg>
    </div>
  );
...
function CornellLayout({ accent, styles }: { accent: string; styles: React.CSSProperties }) {
  const textColor = (styles.color as string) || "#1a1916";
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
      {/* Cue column divider */}
      ...
      {Array.from({ length: 18 }).map((_, i) => (
        <div key={i} style={{ position: "absolute", left: "152px", right: "32px", top: `${106 + i * 24}px`, height: "1px", background: accent, opacity: 0.1 }} />
      ))}
      {/* Ruled lines in cue area */}
      {Array.from({ length: 18 }).map((_, i) => (
        <div key={i} style={{ position: "absolute", left: "32px", width: "100px", top: `${106 + i * 24}px`, height: "1px", background: accent, opacity: 0.1 }} />
      ))}
    </div>
  );
}

function WeeklyLayout({ accent, styles }: { accent: string; styles: React.CSSProperties }) {
        ...
        {days.map(day => (
          <div key={day} style={{ border: `1px solid ${accent}`, borderRadius: "6px", opacity: 0.5, overflow: "hidden" }}>
            ...
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} style={{ borderBottom: `1px solid ${accent}20`, height: "20px" }} />
            ))}
          </div>
        ))}
      </div>
...
const ExportPreview = React.forwardRef<HTMLDivElement, {
  ...
  return (
    <div ref={ref} style={styles[template]}>
      ...
      {/* Layout overlays */}
      {layoutTemplate === "lined"   && <LinedLayout accent={accent} bg={styles[template].background as string} />}
      {layoutTemplate === "grid"    && <GridLayout accent={accent} />}
      {layoutTemplate === "bullet"  && <DotGridLayout accent={accent} />}
      {layoutTemplate === "cornell" && <CornellLayout accent={accent} styles={styles[template]} />}
      {layoutTemplate === "weekly"  && <WeeklyLayout accent={accent} styles={styles[template]} />}
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        {/* For layout templates: show title + day labels, skip body text */}
        {isLayoutOnly && layoutTemplate !== "weekly" ? (
          <>
            ...
            {/* For lined/grid/dot: show faint "write here" hint */}
